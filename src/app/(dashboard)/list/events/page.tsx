  ...
  { classId: null },
  {
    class: {
      [role as keyof typeof roleConditions]: {
        ...
        AND: [{ id: { equals: await getCurrentUserId() } }],
      },
    },
  },
];

    const [eventsData, totalCount] = await prisma.$transaction([
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-4  w-full md:w-auto">
                        ...
                        <div className="flex items-center gap-4 self-end">
                            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-mapYellow" title="Filter">
                                <Image src="/filter.png" alt="Filter" width={14} height={14} />
                            </button>
                            <button className="w-8 h-8 flex items-center justify-center rounded-full bg-mapYellow" title="Sort">
                                <Image src="/sort.png" alt="Sort" width={14} height={14} />
                            </button>
                            {role==="admin" && (<button className="w-8 h-8 flex items-center justify-center rounded-full bg-mapYellow" title="Add Event">
                                <Image src="/plus.png" alt="Add Event" width={14} height={14}/>
                            </button>)}
                        </div>
                    </div>
                </div>
